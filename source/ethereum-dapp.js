...
async function fetch_token_sales(contract, limit, next_string) {
    ...
    fetch('https://api.reservoir.tools/sales/v5?collection=0xBE4Bef8735107db540De269FF82c7dE9ef68C51b?sortBy=time&sortDirection=asc&limit='+limit+next+'', options)
    .then((data) => data.json())
    .then((data) => {
        console.log(data)
...
async function build_token(html_elements, token_id, element_id, txn, txn_hash) {
    ...
    if (txn == 'sale') { var txn_link = '<br><a href="https://etherscan.io/tx/'+txn_hash+'" target="_blank"><button class="secondarySale_button">Secondary Sale</button></a>'; } 
    else if (txn == 'mint') { var txn_link = '<br><a href="https://etherscan.io/tx/'+txn_hash+'" target="_blank"><button class="collectionMint_button">Collection Mint</button></a>'; } 
    else { var txn_link = ''; }
