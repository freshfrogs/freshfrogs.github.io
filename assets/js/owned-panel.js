/* owned-panel.js — Owned/Staked dashboard with:
   - Rank pill color grades (unchanged)
   - NEW: Staking Level + tiny progress bar on staked frogs
   - NEW: Layered/animated render hook via window.FF.renderFrog(canvas, meta, { size:128 })
     Fallback to static /frog/{id}.png when the hook isn’t provided.

   Config knobs (via FF_CFG):
   - STAKE_LEVEL_SECONDS (default: 86400) — seconds required per level
*/
...
(function (FF, CFG) {
  ...
  const PAGE_SIZE = Math.max(1, Math.min(20, Number(CFG.OWNED_PAGE_SIZE || CFG.PAGE_SIZE || 12))); // Reservoir cap is 20
  ...
  const LEVEL_SECS = Math.max(1, Number(CFG.STAKE_LEVEL_SECONDS || 86400)); // 1 day per level by default
...
/* Tighten meta spacing + add level row space */
.frog-card .title{ margin-bottom: 4px; }
.frog-card .meta{ margin-top: 0; color:#22c55e; } /* make "Staked 1130d ago" stand out (green) */
...
/* 128x128 media box to host canvas or img */
...
.frog-card .thumb, .frog-card canvas.frog-canvas{
  ...
  // Level math
  function levelInfo(sinceMs){
    if (!sinceMs) return { level:0, pct:0 };
    const elapsedSec = Math.max(0, Math.floor((Date.now() - sinceMs)/1000));
    const level = Math.floor(elapsedSec / LEVEL_SECS); // Level 0,1,2,...
    const into = elapsedSec % LEVEL_SECS;
    const pct = Math.max(0, Math.min(100, Math.round((into / LEVEL_SECS)*100)));
    return { level, pct };
  }

  // --- Minimal modal infra (unchanged) ---
  ...
  function closeModal(){ const m = document.getElementById('ownedModal'); if (m) m.classList.remove('show'); }
  ...
  function normalizeIds(rows){
    ...
    const toNum=(x)=>{ try{
      ...
      if (Array.isArray(x)) return toNum(x[x.length-1]);
      if (typeof x==='object'){
  ...
  async function getStakedIds(addr){
    ...
    }catch(e){ console.warn('[owned] wallet getStakedTokens failed', e); return []; }
  }
  ...
  async function sendApprove(owner){ await ensureCorrectChain(); const nft = nftContract(); return nft.methods.setApprovalForAll(CFG.CONTROLLER_ADDRESS, true).send({ from: owner }); }
  async function sendStake(owner, tokenId){ await ensureCorrectChain(); const ctrl = ctrlContract(); return ctrl.methods.stake(String(tokenId)).send({ from: owner }); }
  async function sendUnstake(owner, tokenId){ await ensureCorrectChain(); const ctrl = ctrlContract(); return ctrl.methods.withdraw(String(tokenId)).send({ from: owner }); }
  ...
  async function stakeSinceViaEvents(tokenId){
    try{
      ...
      const provider = window.ethereum; if (!provider) return null;
      const web3 = new Web3(provider);
      const erc721 = new web3.eth.Contract([
        {"anonymous":false,"inputs":[{"indexed":true,"internalType":"address","name":"from","type":"address"},{"indexed":true,"internalType":"address","name":"to","type":"address"},{"indexed":true,"internalType":"uint256","name":"tokenId","type":"uint256"}],"name":"Transfer","type":"event"}
      ], CFG.COLLECTION_ADDRESS);
      const evs = await erc721.getPastEvents('Transfer', { filter: { to: CFG.CONTROLLER_ADDRESS, tokenId: tokenId }, fromBlock: 0, toBlock: 'latest' });
  ...
  // --- Metadata cache (store full JSON for renderer) ---
  const META = new Map();
  async function fetchMeta(id){
    ...
    try{
      ...
      const attrs = Array.isArray(j?.attributes)
        ...
        : [];
      const out = { id, attrs, raw: j };
      META.set(id,out); return out;
    }catch{
      const out={ id, attrs:[], raw:null }; META.set(id,out); return out;
    }
  }
  async function loadMetaBatch(ids){ const out=[]; for (const id of ids){ out.push(await fetchMeta(id)); } return out; }
  ...
  function headerData(){ const ownedOnly = Array.isArray(items) ? items.filter(x => !x.staked).length : 0; return { owned: ownedOnly, staked:(_stakedCount==null?'—':_stakedCount), rewards:_rewardsPretty, approved:_approved }; }
  ...
  // --- Panels (approval/stake/unstake) — unchanged text/actions ---
  function openApprovePanel(owner, stats){
    const body = `
      <div class="om-col" style="text-align:center">
        <img class="om-logo" src="assets/img/blackWhite.png" alt="Fresh Frogs logo">
        <div class="om-name">Fresh Frogs Staking</div>
        <div class="om-copy" style="text-align:left">
          <p><b>Approve the staking contract</b></p>
          <p>Give permission to the controller contract to manage your Frogs for staking. While staked, Frogs can’t be listed on marketplaces like OpenSea. Unstake anytime directly on this site.</p>
          <p>This is a one-time transaction to begin staking.</p>
        </div>
      </div>
    `;
    openModal({
      ...
      actions:[
        ...
        { label:'Approve Staking', primary:true, keepOpen:true, onClick: async ()=>{
            try{ await sendApprove(owner); toast('Approval sent'); }
  ...
  function openStakePanel(owner, tokenId){
    const body = `
      <div class="om-col">
        ...
        <div class="om-copy"><p>Stake Frog #${tokenId}. While staked, it can’t be transferred or listed. Unstake anytime.</p></div>
      </div>
    `;
    openModal({
      title: '',
      ...
      actions:[
        ...
        { label:`Stake Frog #${tokenId}`, primary:true, keepOpen:true, onClick: async ()=>{
            try{
              ...
              renderCards(); await refreshHeaderStats();
  ...
  function openUnstakePanel(owner, tokenId){
    const body = `
      <div class="om-col">
        <img class="om-thumb" src="${imgFor(tokenId)}" alt="Frog #${tokenId}">
        <div class="om-name">Frog #${tokenId}</div>
        <div class="om-copy"><p>Return Frog #${tokenId} from the staking contract to your wallet. Its staking level resets to 0.</p></div>
      </div>
    `;
    openModal({
      title: '',
  ...
  // Render image area: tries custom renderer, else falls back to png
  function mountMedia(el, it){
    const box = el.querySelector('.thumb-wrap');
    if (!box) return;
    // If a custom renderer exists, draw into canvas
    const meta = it.metaRaw || null;
    const hasRenderer = (typeof FF.renderFrog === 'function') && meta;
    if (hasRenderer){
      ...
      try{
        FF.renderFrog(canvas, meta, { size:128, tokenId: it.id });
      }catch(e){
        console.warn('[owned] renderFrog failed, fallback to PNG', e);
        box.innerHTML = `<img class="thumb" src="${imgFor(it.id)}" alt="${it.id}">`;
      }
    }else{
      box.innerHTML = `<img class="thumb" src="${imgFor(it.id)}" alt="${it.id}">`;
    }
  ...
  function levelRowHTML(it){
    if (!it.staked || !it.sinceMs) return '';
    const li = levelInfo(it.sinceMs);
    return `
  ...
  function renderCards(){
    ...
    if (!items.length){
      ...
      updateHeaderOwned(); syncHeights(); return;
    }
    ...
    items.forEach(it => {
      ...
      const rankP = (it.rank || it.rank === 0) ? rankPill(it.rank) : '';
      const attrs = attrsHTML(it.attrs, 4);

      card.innerHTML = [
        `<div class="thumb-wrap"></div>`,
        `<h4 class="title">Frog #${it.id}${rankP}</h4>`,
        ...
        `<div class="actions">
           ...
           <button class="btn btn-outline-gray" data-act="transfer" ${it.staked ? 'disabled title="Transfer disabled while staked"' : ''}>Transfer</button>
      ...
      const observer = new IntersectionObserver(async (es)=>{
        ...
        try{
          ...
          const more = moreMetas
            ...
            .map(m=> ({ id:m.id, attrs:m.attrs, metaRaw:m.raw, staked: stakedIds.includes(m.id), sinceMs:null, rank:(FF.RANKS||{})[String(m.id)] }));
