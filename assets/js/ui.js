...
export function renderSales(list = salesCache) {
  ...
  const arr =
    list && list.length
      ? list
      : [{ id: 3250, time: "3m", price: "0.080 ETH", buyer: "0x9a…D1" }];
...
export async function loadSalesLive() {
  try {
    if (!FF_CFG.FROG_API_KEY || FF_CFG.FROG_API_KEY === "YOUR_RESERVOIR_API_KEY_HERE") {
      throw new Error("Missing Reservoir API key");
    }
...
function setRarityList(arr) {
  ...
  window.FF_getRankById = (id) =>
    RANK_LOOK ? RANK_LOOK[String(id)] ?? null : null;
}
...
function sortedRarity() {
  ...
  if (sortBy === "score")
    a.sort(
      (x, y) =>
        Number(y.rarity ?? y.score ?? 0) - Number(x.rarity ?? x.score ?? 0)
    );
  else
    a.sort(
      (x, y) =>
        Number(x.ranking ?? x.rank ?? 1e9) - Number(y.ranking ?? y.rank ?? 1e9)
    );
  return a;
}
...
export function renderRarity() {
  ...
  if (!data.length) {
    ul.innerHTML =
      '<li class="list-item"><div class="muted">No data yet</div></li>';
    return;
  }
  ...
  data.forEach((it) => {
    ...
    const score = (it.rarity ?? it.score ?? "").toString();
...
export async function loadRarity() {
  ...
  if (!ok) {
    ...
    b?.addEventListener("click", () => {
      ...
      if (!file) {
        alert("Choose assets/freshfrogs_rarity_rankings.json");
        return;
      }
...
export function setRaritySort(mode) {
  sortBy = mode;
  renderRarity();
}
...
function setWalletUI(a) {
  ...
  if (a) {
    l.textContent = "Connected: " + shorten(a);
    l.style.display = "";
...
export function initWallet({ onConnect, onDisconnect, onChanged } = {}) {
  async function connect() {
    if (location.protocol === "file:") {
      alert("Open the site over http(s) to enable wallet.");
      return;
    }
    const p = window.ethereum;
    if (!p) {
      alert("No Ethereum provider found. Install/enable MetaMask.");
      return;
    }
    try {
      const acc = await p.request({ method: "eth_requestAccounts" });
      currentUser = acc?.[0] || null;
      setWalletUI(currentUser);
  ...
  document
    ...
    ?.addEventListener("click", () => (currentUser ? disconnect() : connect()));

  if (window.ethereum) {
    window.ethereum.on?.("accountsChanged", (a) => {
      currentUser = a?.[0] || null;
      setWalletUI(currentUser);
...
let _getTab = () => "owned";
export function setTabGetter(fn) {
  _getTab = fn;
}
function getTab() {
  return _getTab();
...
export function renderOwned() {
  ...
  if (!user) {
    list.innerHTML =
      '<li class="list-item"><div class="muted">Connect your wallet to view owned tokens.</div></li>';
    return;
  }
  if (!heldTokens.length) {
    list.innerHTML =
      '<li class="list-item"><div class="muted">No tokens loaded yet. Click “Refresh Owned”.</div></li>';
    return;
  }
...
export async function fetchOwned(wallet, limit = 50, nextStr) {
  try {
    ...
    if (!wallet) {
      document.getElementById("stakeStatus").textContent =
        "Connect a wallet to load owned tokens.";
      return;
    }
    ...
    ss.textContent =
      `Owned: ${heldTokens.length}` +
      (heldContinuation ? " • more available" : "");
...
export function wireFeatureButtons() {
  ...
  document.getElementById("fetchLiveBtn")?.addEventListener("click", () =>
    loadSalesLive()
  );
  ...
  document.getElementById("selectAll")?.addEventListener("click", () => {
    document.getElementById("stakeStatus").textContent =
      "Selected all visible tokens (demo).";
  });
  document.getElementById("clearSel")?.addEventListener("click", () => {
    document.getElementById("stakeStatus").textContent =
      "Cleared selection (demo).";
  });
