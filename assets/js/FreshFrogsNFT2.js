  ...
  async function fetch_user_data(fetch_address) {
    ...
    if (user_tokens >= 1 || staker_tokens >= 1) {
      ...
      if (render_vault) {
        ...
        Output(
          '<button class="stake_button" style="background: cornflowerblue;" onclick="claimRewards_init()">'+
            '<acc style="color: #333 !important;">[ '+truncateAddress(fetch_address)+' ]</acc><br>'+staker_tokens+' / '+user_tokens+' Frog(s) Staked '+' | '+staker_rewards+' $FLYZ 🡥'+
          '</button>'+'<br>'+
