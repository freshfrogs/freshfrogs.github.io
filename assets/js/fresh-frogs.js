    ...
    async function connect() {
        ...
        if (owned_frogs <= 0 && staked_tokens <= 0) { // ❌ No FROGS
            ...
            Output('<br><button onclick="claim_rewards()" style="margin: 16px;" class="frog_button">'+'<strong>Connected!</strong> <acc style="color: #333 !important;">[ '+truncateAddress(user_address)+' ]</acc><br>'+staked_frogs+' Frogs(s) Staked '+''+stakers_rewards+' $FLYZ 🡥</button>'+'<br><hr>'+'<div class="console_pre" id="console-pre"></div>'); // '[ '+stakers_rewards+' $FLYZ ] Rewards available <br>'
