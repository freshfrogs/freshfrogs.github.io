    ...
    async function connect() {
        ...
        if (owned_frogs <= 0 && staked_tokens <= 0) { // ❌ No FROGS
            ...
            Output('<br>'+'<strong>Connected!</strong> <acc style="color: #333 !important;">[ '+truncateAddress(user_address)+' ]</acc><br>'+staked_frogs+' Frogs(s) Staked '+'<button onclick="claim_rewards()" class="frog_button">'+stakers_rewards+' $FLYZ 🡥</button>'+'<br><hr>'+'<div class="console_pre" id="console-pre"></div>'); // '[ '+stakers_rewards+' $FLYZ ] Rewards available <br>'
