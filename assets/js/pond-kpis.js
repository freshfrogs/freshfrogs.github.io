...
// If the row isn't present (or request fails), leaves the KPI blank ("—").
...
(function () {
  ...
  var CONTROLLER = CONTROLLERS[0] || ''; // primary controller
  var RES_KEY = CFG.RESERVOIR_API_KEY || null;

  // (Optional) safe fallbacks if CFG not ready yet
  ...
  var HEAD = { accept:'*/*' };
  if (RES_KEY) HEAD['x-api-key'] = RES_KEY; // never use demo keys
  ...
  function fetchOwners(){
    // Big limit + sort so the controller row is guaranteed to be included.
  ...
  function extractControllerCount(json){
    // Accept both shapes: j.owners[] or j.ownerships[]
  ...
  function fillTotal(){
    ...
    fetchOwners()
      .then(function(json){
        ...
        out.textContent = (count == null) ? '—' : fmt(count);
      })
      .catch(function(){ out.textContent = '—'; });
  }

  function init(){ setLabel(); setControllerBox(); setFLYZ(); removeFourthBox(); fillTotal(); }
