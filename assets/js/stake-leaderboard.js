...
// Staking Leaderboard (last N days) — counts "stake" actions by wallet using the same Reservoir pipeline as stakes-feed.
// Renders into <ol id="stakeLeaderboard"> with simple, clean rows.
...
(function(FF, CFG){
  ...
  // Tuning
  const LOOKBACK_DAYS = Number(CFG.LB_LOOKBACK_DAYS || 30);
  const PAGE_SIZE     = Math.max(1, Math.min(50, Number(CFG.LB_PAGE_SIZE || 25)));
  const MAX_PAGES     = Math.max(1, Number(CFG.LB_MAX_PAGES || 20));  // 20*25 = 500 recent items
  const TOP_N         = Math.max(3, Number(CFG.LB_TOP_N || 10));

  if (!API_KEY || !CONTROLLER || !COLLECTION){
    console.warn('[leaderboard] missing API key / controller / collection'); 
  }
...
@media (max-width:700px){
  ...
  // ------- Shared fetch queue (reuse if present) -------
  ...
  function mapRow(a){
    ...
    return { id: tokenId, kind, other, tms };
  }

  async function fetchPage(continuation){
    const qs = new URLSearchParams({ users: CONTROLLER, collection: COLLECTION, types:'transfer', limit:String(PAGE_SIZE) });
  ...
  async function build(){
    ...
    while(page < MAX_PAGES){
      ...
      for (const r of rows){
        if (!r || r.tms==null || r.tms < sinceMs) continue; // respect lookback window
        if (r.kind !== 'stake') continue;                   // count stakes only
        if (!r.other) continue;
        const key = r.other.toLowerCase();
