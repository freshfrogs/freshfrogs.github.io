(() => {
  ...
  const FF = window.FF;
  if (!FF) {
    console.error("FF core not loaded before ff-cards.js");
    return;
  }
  ...
  function ffAddTraitLayerAnimatedFirst(traitType, value, container, containerId) {
    ...
    if (typeof SOURCE_PATH === "undefined") {
      // no build files system, bail to static
  ...
  async function ffBuildLayeredFrogImage(tokenId, containerId) {
    ...
    try {
      ...
      // if no trait system, just show base
      if (typeof SOURCE_PATH === "undefined") {
        const base = new Image();
        base.src = baseUrl;
        base.className = "recent_sale_img";
        base.alt = `Frog #${tokenId}`;
        base.loading = "lazy";
  ...
  async function ffBuildLayeredMorphedImage(metadata, containerId, baseTokenId = null) {
    ...
    try {
      ...
      if (typeof SOURCE_PATH !== "undefined" && attrs.length) {
        for (const a of attrs) {
          if (!a?.trait_type || a?.value == null) continue;
          ffAddTraitLayerAnimatedFirst(a.trait_type, a.value, container, containerId);
        }
        return;
      }
