...
(function () {
  ...
  // Values (attribute "value") that have animation variants
  ...
  style.textContent = `
    ...
    /* Snake skin stages – subtle color shifts */
    #frog-bg.snake-stage-1 .snake-head,
    #frog-bg.snake-stage-1 .snake-body,
    #frog-bg.snake-stage-1 .snake-tail {
      filter: none;
    }

    #frog-bg.snake-stage-2 .snake-head,
    #frog-bg.snake-stage-2 .snake-body,
    #frog-bg.snake-stage-2 .snake-tail {
      filter: hue-rotate(25deg) saturate(1.2);
    }

    #frog-bg.snake-stage-3 .snake-head,
    #frog-bg.snake-stage-3 .snake-body,
    #frog-bg.snake-stage-3 .snake-tail {
  ...
  let scrollOffsetY = 0; // no parallax, but kept for compatibility
  let lastScrollY   = 0;
  ...
  function updateSnakeSkinStage() {
    if (!SNAKE_ENABLED) return;

    container.classList.remove("snake-stage-1", "snake-stage-2", "snake-stage-3");

    let stage = 1;
    if (frogsEaten >= 15) stage = 3;
    else if (frogsEaten >= 5) stage = 2;

    container.classList.add("snake-stage-" + stage);
  }
  ...
  function createFrogAtPosition(pos, tokenId) {
    const x = pos.x;
    const y = pos.y;
  ...
  async function createFrogs(width, height) {
    ...
    for (let i = 0; i < positions.length; i++) {
      createFrogAtPosition(positions[i], tokenIds[i]);
    }
  }
    ...
    for (const frog of frogs) {
      if (frog.state === "idle") {
        ...
        if (frog.idleTime <= 0) {
          ...
          // --- DIRECTION: follow mouse when enabled, otherwise random ---
          let angle;
          if (mouse.follow && mouse.active) {
            const cx = frog.x + FROG_SIZE / 2;
            const cy = frog.baseY + FROG_SIZE / 2;
            const dx = mouse.x - cx;
            const dy = mouse.y - cy;
            angle = Math.atan2(dy, dx);

            // small random spread so they don't stack perfectly
            angle += randRange(-0.45, 0.45);
          } else {
  ...
  function initSnake(width, height) {
    ...
    snakeEatSound = new Audio("audio/munch.mp3");
    snakeEatSound.volume = 0.8;

    frogsEaten = 0;
    updateSnakeSkinStage();
  }
  ...
  function updateSnake(dt, width, height) {
    ...
    head.el.style.transform =
      `translate3d(${head.x - SNAKE_SEGMENT_SIZE / 2}px, ${head.y - SNAKE_SEGMENT_SIZE / 2}px, 0) rotate(${head.angle}rad)`;
    ...
    for (let i = 1; i < snake.segments.length; i++) {
      ...
      seg.el.style.transform =
        `translate3d(${seg.x - SNAKE_SEGMENT_SIZE / 2}px, ${seg.y - SNAKE_SEGMENT_SIZE / 2}px, 0) rotate(${seg.angle}rad)`;
    }
  ...
  function spawnOrb(width, height) {
    ...
    // Only speed + jump buffs now
    const buffTypes = ["speed", "jump"];
    const buffType = buffTypes[randInt(0, buffTypes.length - 1)];

    let hue;
    if (buffType === "speed") hue = 50;      // warm yellow/orange
    else hue = 300;                          // magenta (jump)
  ...
  function updateOrbs(dt, width, height) {
    ...
    for (const orb of orbs) {
      orb.el.style.transform =
        `translate3d(${orb.x - orb.r}px, ${orb.y - orb.r}px, 0)`;
    }
  ...
  window.addEventListener("click", (e) => {
    ...
    // once you click, frogs start biasing hops toward the mouse
    mouse.follow = true;
    ...
    if (nearest && nearestDist2 <= MAX_CLICK_RADIUS * MAX_CLICK_RADIUS) {
      if (SNAKE_ENABLED) {
        playFrogJumpSound(true); // force ribbit on click
      }
