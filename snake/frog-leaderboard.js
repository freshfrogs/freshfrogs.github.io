...
(function () {
  ...
  async function submitScoreToServer(score, time) {
    try {
      ...
      const data = await res.json();
      // Allow either raw array or { entries: [...] }
      if (Array.isArray(data)) return data;
      if (data && Array.isArray(data.entries)) return data.entries;
      return null;
    } catch (err) {
  ...
  function openScoreboardOverlay(topList, lastRunScore, lastRunTime) {
    ...
    // Find the entry that matches this run (so we can highlight name)
    let myIndex = -1;
    let myEntry = null;
    ...
    if (Array.isArray(topList)) {
      ...
      for (let i = 0; i < topList.length; i++) {
        const e = topList[i];
        const es = typeof e.score === "number" ? e.score : 0;
        const et = typeof e.time === "number" ? e.time : 0;
